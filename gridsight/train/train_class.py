...
def fit_class(asset_class: str, cfg: PatchcoreConfig) -> tuple[AnomalyModule, dict[str, float | None]]:
    ...
    # No ground-truth masks ship with these Hub datasets, so pixel AUROC is
    # genuinely unmeasurable here -- recorded as null rather than invented.
    # For the same reason anomalib leaves `_pixel_threshold` as NaN; calibrate it
    # from the training set's own anomaly maps so heatmaps and region extraction
    # actually work instead of silently producing NaN.
...
#: A model's routing gate is the Nth percentile of its own training images'
#: similarity to its centroid -- i.e. the coverage envelope it actually learned.
#: Measured over the seeded registry, this admits 172/176 in-registry frames while
#: refusing 36/40 out-of-registry ones; the best single global threshold managed
#: only 136/176 at comparable refusal.
...
def routing_embedding(asset_class: str) -> tuple[np.ndarray, int, Path, float]:
    ...
    c = centroid(vectors)
    similarities = vectors @ c
    golden_idx = int(np.argmax(similarities))
    # Mirror Atlas's cosine scoring, (1 + cos) / 2, so the gate is directly
    # comparable to a $vectorSearch score.
    atlas_scores = (1.0 + similarities) / 2.0
    gate = float(np.percentile(atlas_scores, ROUTING_GATE_PERCENTILE))
...
def main() -> int:
    ...
    print("\n" + "=" * 112)
    print(
        f"{'ASSET CLASS':<20}{'WEIGHTS MB':>11}{'EMB':>5}{'SAMPLES':>9}{'IMG AUROC':>11}"
    ...
    for name in sorted(final.get("results", {})):
        ...
        rt = r["roundtrip"]
        auroc = r["metrics"]["image_auroc"]
        print(
            f"{name:<20}{r['weights_mb']:>11.3f}{r['embedding_dim']:>5}{r['training_samples']:>9}"
            f"{(f'{auroc:.4f}' if auroc is not None else 'n/a'):>11}"
